...
**macOS desktop app, Stages A+B (CRAFT-6187):** a Tauri v2 shell (`src-tauri/`) builds against system WebKit and spawns `openklip serve` as a Bun sidecar. `scripts/prepare-desktop-bundle.ts` stages the built `.next`, a production-pruned `node_modules`, `src/`, and asset dirs into `Resources/app/` with a vendored Bun binary, so the `.app` runs with no repo checkout. The sidecar spawn uses plain `std::process::Command` with an atomic pre-exec `.process_group(0)` so quitting the app kills the whole process group (including the `next start` grandchild) instead of orphaning it. Writable state (`.openklip`, model cache) relocates to Application Support/Caches via `stateDir()` (`src/repo-paths.ts`); dev mode is unchanged. The signing/notarization pipeline is scaffolded (`src-tauri/entitlements.plist`, `scripts/verify-macos-signature.sh`, `docs/desktop-packaging-runbook.md`) but Stage C (actual Developer ID sign/notarize) is human-only and not done. The Rust build is not in CI yet.
