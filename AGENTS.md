...
| Manage background jobs (cancel / retry / clean up) | GUI Config sidebar **Jobs** tab; `GET /api/projects/jobs`, `GET /api/projects/<slug>/silences/jobs`, per-job cancel/retry/delete routes (no CLI verb yet) |
| Reclaim disk / rehydrate a compacted project | GUI Config → Project **Disk** section; `POST`/`GET /api/projects/<slug>/compact`, `POST /api/projects/<slug>/rebuild` (starts a Job Center job; no CLI verb yet) |
...
**Editor code-splitting (CRAFT-6172):** heavy client stacks load on demand via `next/dynamic` (`ssr: false`): graphic overlays + Paper shaders (`preview-overlays` → `graphic-overlay` / runtime), map-motion / maplibre (`json-render-graphic-overlay` → `map-motion-frame`), template hover previews (do not static-import `graphic-template-preview` for chrome/captions), and media audio visualizers (wave shader split out of LiveKit agent visualizer). Timeline graphic types live in `web/components/graphic-item.ts` so the shell type-imports stay free of shader deps. Prefer dynamic import for new niche panel deps rather than static imports from the editor shell.
...
**History snapshots (CRAFT-6174):** logged mutations write gzipped pre-edit snapshots as `working/history/rev-<n>.json.gz` (legacy plain `.json` still loads). Prune/list treat both suffixes as one revision.
...
**Loopback bind + local trust boundary (CRAFT-6175):** `openklip serve`/`dev` binds Next's dev server to `127.0.0.1` by default (`src/cli.ts`); set `OPENKLIP_HOST` to override for deliberate LAN access (prints a security warning when the value is not loopback). `src/local-trust.ts`'s `trustGuard` is applied to every mutating API route handler (27 route files, 29 call sites as of v0.44.0, measured via `grep -rln "trustGuard(" app/api`, including the job cancel/retry/delete routes and the newer compact/rebuild routes): it rejects a request carrying positive browser evidence of being off-device or cross-site (non-loopback Host, cross-origin Origin, cross-site Sec-Fetch-Site) with HTTP 403, and stays permissive for same-origin browser calls and non-browser CLI/MCP callers (which omit those headers). Server action failures (`fail()` in `app/actions.ts`) attach a stack trace only when `OPENKLIP_DEBUG=1`; previously any non-production `next dev` launch leaked stacks to the browser by default. A future LAN-access opt-in needs a per-launch token, not a wider header allowlist; not implemented in this pass.

**Atomic project.json writes (CRAFT-6176):** `saveProject` (`src/projectStore.ts`) validates via `ProjectSchema.parse`, writes to a same-directory unique temp file (`project.json.tmp-<pid>-<uuid>`), `fsync`s it, then atomically renames over `project.json`, preserving the existing file's permission bits and unlinking the temp file on any failure; stale `project.json.tmp-*` siblings left by a prior crash are swept on the next save. No public API/CLI/flag change; a crash mid-write can no longer leave the only EDL truncated or unloadable.

**Required golden-path browser test (CRAFT-6184):** `tests/golden-path-browser.test.ts` boots a real production `next start` server and drives cut → server-action persistence → `GET /api/projects/<slug>/history` → reload-survives-cut on disk → export via the real UI → structural ffprobe verification → a controlled 4xx export failure, with page/console/network/hydration error capture. Registered in `test:integration` and the local pre-push gate alongside the other browser tests.

**Transactional force re-ingest (CRAFT-6181):** `src/ingest-swap.ts` stages a `--force` re-ingest under a throwaway sibling slug, validates the staged `project.json`, then swaps it into the live slug under the same lock `mutateProject` uses (`rename` live→backup, `rename` staging→live; a failed second rename restores from backup). Refuses when an agent task is `running`/`pending` on the slug, checked before staging and again right before the swap. Non-force `ingest` is byte-identical to before.

**Streamed asset uploads (CRAFT-6179):** the asset upload route streams straight to disk via `writeUploadToFile` (`@engine/upload-stream`) instead of `file.arrayBuffer()`, closing an OOM path on large files; 413s both on the declared multipart size before streaming and on the bytes actually written, with cleanup on every failure path.

**Mobile Config access restored (CRAFT-6178):** at a 390px viewport an "Open config" mobile header button (mirroring the existing Open Chat button) opens the left sidebar's Config Sheet, which previously had no reachable trigger on mobile. `test:integration` now lists and runs all browser tests so mobile regressions are caught by the local pre-push gate.

**Overlay save concurrency (CRAFT-6177):** overlay add/update/remove route through ID-scoped registry actions (`web/lib/use-overlay-editors.ts`) instead of a whole-track replacement built from a client snapshot, so a concurrent CLI/MCP edit on the same track survives by construction; b-roll reorder (genuinely whole-track) keeps a CAS check (`expectedRevision`) rejecting a stale reorder before any write.

**Optimistic-save recovery (CRAFT-6182):** `web/lib/save-queue.ts` is a state machine (`queued → persisting → persisted|failed → retrying`) with an exactly-once `beginPersisting` gate; `SaveRecoveryBanner` shows a persistent error plus dirty count with Retry and a confirm-gated Reload-from-disk, and `web/app.tsx` adds `beforeunload`/navigation guards while unsaved work remains.

**Pipe-safe large JSON output (CRAFT-6180):** `tools --json`, `actions --json`, and `features --json` route through a drain-aware `writeStdoutJson()` that awaits the `drain` event before returning, fixing silent truncation of the ~92KB tool manifest at Bun's 65,536-byte pipe buffer.

**Durable background jobs (CRAFT-6183, CRAFT-6253, CRAFT-6263, CRAFT-6264):** `src/job-store.ts` persists ingest and silences jobs to disk (atomic tmp+rename+fsync, corruption backup-and-recover, once-per-restart reconciliation to an `"interrupted"` status, terminal-only retention cap), so a restart no longer loses an in-flight job. `AbortSignal` threads through the whole-project ingest pipeline (`src/ffmpeg.ts`'s `run()`, `src/ingest.ts`, `src/moment-search.ts`, cooperative checks in `src/audio-analysis.ts`) for real cancellation, plus an exactly-once retry guard and terminal-only clean-up (removes only the job record, never the project directory). Take/cam ingest jobs are cancellable too (`ffprobeJson`/`probe`/`probeAudio` accept an optional `signal`; `ingestTake`/`ingestCam` thread it through their phases); `ingestCam` now accepts an `onProgress` callback too (CRAFT-6263, PR #144), mirroring the take pipeline's probe → proxy → audio emit convention, so cam jobs report phase text like every other job kind. `openTargetSlug(job)` (`web/components/jobs-panel.tsx`, CRAFT-6264, PR #145) strips a take/cam job's composite `${parent}/takes/${id}`/`${parent}/cams/${id}` slug so Open lands on the parent project instead of a dead route. New routes: `GET /api/projects/jobs` (workspace-wide ingest list), `GET /api/projects/<slug>/silences/jobs`, and per-job cancel/retry/delete under both `ingest/<jobId>/` and `<slug>/silences/<jobId>/`, all mutating routes behind `trustGuard`. GUI: Config sidebar **Jobs** tab (`web/components/jobs-panel.tsx`), 2s polling only while a job is running.

**Project disk management (CRAFT-6265):** `src/compact-project.ts`'s `compactProject(slug)` (`withProjectLock`-guarded) deletes regenerable derived media (proxy, `audio16k.f32`, frames/, moment-index.json, transcript.json, audio-analysis.json, output/) while keeping `project.json`, `brief.md`, `assets/`, and every user-edit-class `working/` file; `src/rebuild-project.ts`'s `rebuildProjectMedia(slug)` reruns the ingest media pipeline against `project.json`'s `source` as a Job Center job to restore playback. `POST`/`GET /api/projects/[slug]/compact` and `POST /api/projects/[slug]/rebuild` (POST routes behind `trustGuard`); GUI Config → Project **Disk** section (`web/components/project-disk-panel.tsx`) arms/confirms both with a "needs rebuild" banner. No CLI verb yet; does not check for an in-flight job on the same slug before compacting.

**Production launcher (CRAFT-6185):** `openklip serve` runs a production `next start` (requires a prior `bun run build`; fails with an actionable message otherwise); `openklip dev` keeps `next dev`. `appRoot()` (`src/repo-paths.ts`, `OPENKLIP_APP_ROOT` env override) resolves read-only bundled assets against a distribution-relative base instead of a hardcoded checkout; writable `.openklip` config stays cwd-relative outside the desktop app. `openklip doctor` gained an `app-root` check. New `src/serve-runtime.ts` gives an actionable port-conflict message instead of a stack trace.

**Deterministic media acceptance gate (CRAFT-6186):** `scripts/acceptance-corpus.ts` generates 5 reproducible ffmpeg-lavfi fixtures into gitignored `fixtures/acceptance/`; `scripts/acceptance-gate.ts` runs real `ingest()` → a canonical dead-air cut → `exportCut()` → ffprobe structural verification per fixture, gating on deterministic facts (codecs, container, dimensions, frame rate, duration within a tolerance band, A/V presence, no ffmpeg/ingest error) via ffprobe. Full gate behind `OPENKLIP_ACCEPTANCE=1`; local CI runs it offline after warming the model cache.

**macOS desktop app (CRAFT-6187):** a Tauri v2 shell (`src-tauri/`) builds against system WebKit and spawns `openklip serve` as a Bun sidecar. `scripts/prepare-desktop-bundle.ts` stages the built `.next`, a production-pruned `node_modules`, `src/`, and asset dirs into `Resources/app/` with a vendored Bun binary, so the `.app` runs with no repo checkout. The sidecar spawn uses plain `std::process::Command` with an atomic pre-exec `.process_group(0)` so quitting the app kills the whole process group (including the `next start` grandchild) instead of orphaning it. Writable state (`.openklip`, model cache) relocates to Application Support/Caches via `stateDir()` (`src/repo-paths.ts`); dev mode is unchanged. The Rust build is not in the local CI gate yet.

**Stage C: signed, notarized, downloadable (CRAFT-6262, PR #147, PR #153):** `scripts/sign-desktop-bundle.sh` deep-signs every Mach-O bundled inside `Contents/Resources/app/node_modules` (ffmpeg, ffprobe, onnxruntime, sharp, next-swc, esbuild, and more) by file content with Developer ID + hardened runtime + secure timestamp, since `tauri build`/`codesign --deep` only reaches the app shell and `Contents/MacOS/*`. `docs/desktop-packaging-runbook.md` documents the full build → deep-sign → repackage DMG → notarize → staple flow. Every `codesign` call routes through a `sign()` helper: `… </dev/null` (PR #153 fixed a stdin-consumption bug that silently left 12 of 13 nested binaries unsigned on every real rebuild after #147 shipped) plus 5 retries with linear backoff and an exit-code check propagated via `|| exit 1` (PR #156: `--timestamp` hits Apple's timestamp server, which throttles under the ~13-binary burst; a throttled sign used to fail silently and leave the binary unsigned until notarization caught it). Only an `aarch64` (Apple Silicon) build is produced; no Intel/universal binary yet. Each release must upload the DMG under both the versioned name and the version-less `OpenKlip-macos-arm64.dmg` alias the marketing download button links to (`docs/desktop-packaging-runbook.md` "Publish the DMG"), or `releases/latest/download/OpenKlip-macos-arm64.dmg` 404s. The signed, notarized, stapled DMG is downloadable from the [Releases page](https://github.com/craftled/openklip/releases); `content/docs/download-install.mdx` (CRAFT-6266, PR #149) documents install/first-run/uninstall.

**In-app auto-update (CRAFT-6266, PR #151, PR #152):** `tauri-plugin-updater` + `tauri-plugin-dialog`, driven entirely from Rust (`spawn_update_check` in `src-tauri/src/main.rs`, since the webview loads a remote localhost URL with no JS updater to call): checks `plugins.updater.endpoints` (a GitHub Releases `latest.json` feed) on launch, prompts on a newer signed version, then downloads/installs/relaunches. Dormant and best-effort: no published feed, offline, or a malformed manifest are logged and ignored, never a crash or blocked launch. Updater public key committed in `tauri.conf.json`; the passphrase-protected private key lives in 1Password and is needed only at release time (as `TAURI_SIGNING_PRIVATE_KEY`) to sign update artifacts. `createUpdaterArtifacts` is intentionally not enabled in the committed config.

**Engine log file (CRAFT-6266, PR #148):** the sidecar's stdout and stderr are piped and tee'd to a rotating log file under `app.path().app_log_dir()` (`~/Library/Logs/com.craftled.openklip/openklip-engine.log` on macOS; previous run rotates `.log` → `.log.1` → `.log.2`); a non-zero-exit or never-became-ready run additionally retains `openklip-engine.crash-<exitcode>.log` (most recent only). Local disk only, no telemetry; best-effort (an unwritable log dir is skipped silently, never blocks launch).
...
**Browser integration tests:** `tests/json-graphic-browser.test.ts`, `tests/transcript-diff-browser.test.ts`, and `tests/mobile-overlays-browser.test.ts` skip unless `OPENKLIP_INTEGRATION=1` and Chrome are present. Run with `OPENKLIP_INTEGRATION=1 bun test tests/mobile-overlays-browser.test.ts` (set `OPENKLIP_CHROME_PATH` when needed). The local CI runner discovers Chrome on macOS/Linux, hard-fails when none is available, and runs the complete browser suite before push.
...
`agent-smoke-audit`: deterministic agent-loop smoke (no LLM). Local CI runs the lavfi fixture; `--real` audits `edgaras-raw` when present; `--revise` runs the revise-draft cut/title/revert loop; `--all` runs both fixtures plus revise.
...
- README, AGENTS.md, CLAUDE.md, CHANGELOG.md, and other docs
...
**README = what exists in code today.** Philosophy and principles should describe implemented behavior. Roadmap, aspirations, todos, bugs, and post-MVP items do not live in the repo at all: they belong in the **OpenKlip Linear project** (team Craftled, `https://linear.app/craftled/project/openklip-687f57863d8c`). README/CLAUDE may summarize durable by-design limitations in prose but point actionable gaps to Linear. Release bodies live only in GitHub Releases (written via `gh` at release time), not in a repo file.
...
Durable, non-obvious notes for cloud agents. The startup update script already runs `bun install`. Standard commands live in `package.json` scripts (`dev`, `build`, `start`, `serve`, `test`, `typecheck`, `check`, `ci`) and are documented above; only the caveats below are worth remembering. `bun run ci` is the complete macOS/Linux gate and the repository-managed `.githooks/pre-push` runs it automatically. `bun install` configures the checkout's `core.hooksPath`; use `bun run hooks:install` to reapply it. GitHub Actions does not run CI for this repository.
