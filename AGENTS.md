...
**Required golden-path browser test (CRAFT-6184):** `tests/golden-path-browser.test.ts` boots a real production `next start` server and drives cut → server-action persistence → `GET /api/projects/<slug>/history` → reload-survives-cut on disk → export via the real UI → structural ffprobe verification → a controlled 4xx export failure, with page/console/network/hydration error capture. Registered in `test:integration` and the CI `integration` job alongside the other browser tests.
