...
| `openklip broll-add <slug> <assetId> <fromSec> <toSec>` | Cover a source-time span with a registered asset. `--display cover\|pip` (default `cover`; `pip` keeps the speaker visible with an inset). `--audio-mode silent\|broll\|mix\|duck-voice\|duck-broll` (default `silent`). |
| `openklip broll-add-phrase <slug> <assetId> "spoken phrase"` | Cover the span of the first spoken phrase match. |
| `openklip broll-set <slug> <brollId>` | Patch b-roll: `--asset`, `--from`, `--to`, `--src-in` (seconds), `--display cover\|pip`, `--audio-mode silent\|broll\|mix\|duck-voice\|duck-broll`. |
