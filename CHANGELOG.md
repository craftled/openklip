...
- **Linear is the single source of truth for todos, bugs, and roadmap** (PR #159): repo-side checklists no longer duplicate the OpenKlip Linear project.
- **Audited dependency refreshes** (PR #161, PR #162): a safe batch plus a Next.js security patch, then `glimm`, `biome`, and `ultracite`, each landed after test, integration, and acceptance gates passed.

### Fixed
- **Release signing no longer reports false verification failures** (PR #164): the verify step now consumes the full `codesign` output, so `pipefail` cannot turn a genuinely verified Developer ID signature into a spurious failure.
- **DMG conversion finds its temporary image** (PR #165): `hdiutil` temporary images are given an explicit `.dmg` suffix, which conversion requires.
...
### Added
- **The macOS app is now signed, notarized, and downloadable** (PR #147, PR #153, CRAFT-6262): v0.43.0 is signed as Developer ID Application: Craftled, MB, notarized by Apple, and stapled (DMG + app inside); `spctl` accepts it. `scripts/sign-desktop-bundle.sh` enumerates every Mach-O bundled deep in `Contents/Resources/app/node_modules` by file content (not the executable bit, which the bundle copy strips off some binaries) and signs each with hardened runtime + secure timestamp, since `tauri build --deep`/`codesign --deep` never reaches `node_modules`; `docs/desktop-packaging-runbook.md` was rewritten for the real Stage C flow (Account-Holder-only cert creation, the missing G2 intermediate install, `notarytool` keychain-profile credentials, build → deep-sign → repackage DMG → notarize → staple, including the extra staple pass for the app inside the DMG). A follow-up fix (#153) found the sign loop's `codesign` call was consuming the `find`-driven while-loop's stdin (missing the `</dev/null` guard the verify loop already had), silently leaving 12 of 13 nested binaries unsigned on every real rebuild since #147 shipped; the DMG is now downloadable from the [Releases page](https://github.com/craftled/openklip/releases).
- **Download & install docs page** (PR #149, CRAFT-6266): `content/docs/download-install.mdx` covers getting the DMG from GitHub Releases, install and first-run (local engine splash, one-time Whisper/CLIP model download), where app data lives, and uninstall; linked from README Quick start and a new Download nav link on the marketing site.
- **In-app auto-update** (PR #151, PR #152, CRAFT-6266): `tauri-plugin-updater` + `tauri-plugin-dialog`, driven entirely from Rust (`spawn_update_check` in `main.rs`) since the webview loads a remote localhost URL with no JS updater to call. On launch it checks `plugins.updater.endpoints` (the repo's `latest.json` on GitHub Releases); on a newer signed version it prompts, then downloads, installs, and relaunches. Best-effort and dormant by default: no feed published, offline, or a malformed manifest are all logged and ignored, never a crash or blocked launch. The updater public key is committed in `tauri.conf.json`; the production passphrase-protected private key lives in the team's 1Password and is required only at release time to sign update artifacts (#152 swapped out the passphrase-less scaffolding key generated during #151).
- **Project Compact + Rebuild** (PR #150, CRAFT-6265): a new Compact action (`src/compact-project.ts`, `POST /api/projects/[slug]/compact`) deletes regenerable derived media (proxy, `audio16k.f32`, frames/, moment-index.json, transcript.json, audio-analysis.json, output/) to reclaim disk while keeping `project.json`, `brief.md`, assets/, and every user-edit-class working/ file; a paired Rebuild action (`src/rebuild-project.ts`, `POST /api/projects/[slug]/rebuild`) reruns the ingest media pipeline as a Job Center job to restore playback, since the proxy/frames media routes 404 on a missing file rather than lazily rebuilding it. New Config → Project **Disk** section (`web/components/project-disk-panel.tsx`) arms/confirms both actions and shows a "needs rebuild" banner.
- **Engine log file + crash retention** (PR #148, CRAFT-6266): the bundled `openklip serve` sidecar's stdout (previously inherited, invisible from a Finder launch) is now piped alongside the existing piped stderr and both are tee'd to a rotating log file under `app_log_dir()` (`~/Library/Logs/com.craftled.openklip/openklip-engine.log` on macOS; previous run rotates `.log` → `.log.1` → `.log.2`). A run that exits non-zero or dies before the server becomes ready has its log additionally retained as `openklip-engine.crash-<exitcode>.log` (most recent only). Everything stays local; no telemetry. Logging is best-effort throughout: an unwritable log dir is skipped silently and never blocks launch.

### Fixed
- **Cam ingest jobs report phase progress in the Job Center** (PR #144, CRAFT-6263): `ingestCam` (`src/cams.ts`) now accepts an `onProgress` callback and the cams route wires it through `startIngestJob`, mirroring the take pipeline's probe → proxy → audio emit convention (1/3 step/total). Cam jobs previously sat in the Job Center with no phase updates while take and whole-project ingest jobs reported normally.
- **Job Center "Open" on take/cam jobs navigates to the parent project** (PR #145, CRAFT-6264): take/cam ingest jobs carry a composite `job.slug` (`${parent}/takes/${id}` or `${parent}/cams/${id}`), so Open previously pushed a bogus `/?slug=project/takes/abc` route. A new pure `openTargetSlug(job)` helper (`web/components/jobs-panel.tsx`) strips the composite suffix so Open lands on the parent project.
- **Reliable release build + DMG** (PR #146, CRAFT-6261): the release macOS build failed where debug always worked, for two reasons, neither a real "DMG hang." Building under a filesystem sandbox corrupted the parallel Rust compile (intermittent `E0463` proc-macro/dylib errors); the release build now runs non-sandboxed. `tauri build` also recompiles `tauri_macros` under the production `custom-protocol` feature, and the aggressive `[profile.release]` settings (`lto = true`, `codegen-units = 1`, `strip = true`) produced proc-macro dylibs rustc could not load; a new `[profile.release.build-override]` (`opt-level = 0`, `codegen-units = 16`, `strip = false`) compiles host build tools plainly without touching the shipped binary's optimization. With both fixed, `tauri build --bundles app,dmg` produces a valid, `hdiutil`-verify-clean DMG. Desktop shell version bumped 0.42.0 → 0.43.0 to match the tag.

### Documentation
- AGENTS.md documents the Disk (Compact/Rebuild) capability, the engine log file location, and the fixed cam-progress and desktop-signing notes inline. TODO.md Known Limitations and Completed checklist, README verified counts, CLAUDE.md release memory, and `docs/RELEASE-NOTES.md` are synced for this tag.

## 0.43.0 - 2026-07-17

### Added
- **macOS desktop app (Tauri v2)** (PR #135, PR #137, PR #138, CRAFT-6187): a Tauri v2 shell builds against system WebKit (no bundled Chromium). Stage A (#135) spawns `openklip serve` (the production launcher) as a Bun sidecar behind an instant splash, wires the hardened-runtime/entitlements signing pipeline (`src-tauri/entitlements.plist`, `scripts/verify-macos-signature.sh`, `docs/desktop-packaging-runbook.md`), and de-risks WKWebView against real editor/WebGL/MediaSource playback. Stage B (#137) makes the `.app` self-contained: `scripts/prepare-desktop-bundle.ts` stages the built `.next`, a production-pruned `node_modules`, `src/`, and asset dirs into `Resources/app/` with a vendored Bun binary; the sidecar spawn moved off `tauri-plugin-shell` onto plain `std::process::Command` with an atomic pre-exec `.process_group(0)` so quitting the app kills the whole process group (including the `next start` grandchild) instead of orphaning it; writable state (`.openklip`, model cache) relocates to Application Support / Caches via a new `stateDir()` (`src/repo-paths.ts`), while dev mode is unchanged. PR #138 replaces the placeholder app icon with the real OpenKlip glyph rendered through `tauri icon`. Stage C (human-only Developer ID signing/notarization) is not done; see Known Limitations.
- **Production launcher for `openklip serve`** (PR #133, CRAFT-6185): `serve` now runs a production `next start` (requires a prior `bun run build`; fails fast with an actionable message otherwise) instead of `next dev`, so no dev overlay, HMR, or watcher runs in a shipped launch; `openklip dev` keeps `next dev` for contributors. New `appRoot()` (`src/repo-paths.ts`, `OPENKLIP_APP_ROOT` env override) resolves read-only bundled assets (Next binary, ffmpeg/ffprobe, scripts, templates) against a distribution-relative base instead of a hardcoded checkout; writable `.openklip` config stays cwd-relative. New `src/serve-runtime.ts` (spawn-plan + port-preflight helpers) gives an actionable "port in use" message instead of a stack trace, and `openklip doctor` gained an `app-root` check.
- **Empty-workspace serve + engine startup diagnostics** (PR #140): `serve`/`dev` with no slug argument now starts unpinned against an empty workspace (serving the onboarding page) instead of throwing `no projects found`; an explicitly named missing project still errors. The Tauri shell pipes sidecar stderr (bounded 4KB tail) and swaps the splash spinner for a visible error plus the engine's last output when the sidecar dies or never becomes ready within 60s, replacing an indefinite silent spinner.
- **Job Center: durable, cancellable, retryable background jobs** (PR #132, PR #139, PR #141, PR #142, CRAFT-6183, CRAFT-6253): ingest and silences jobs now persist to disk (`src/job-store.ts`: atomic tmp+rename+fsync writes, corruption backup-and-recover, once-per-restart reconciliation to a new `"interrupted"` status, terminal-only retention cap) so an app restart no longer loses in-flight jobs or shows "Ingest job lost" (#132). `AbortSignal` now threads through the whole-project ingest pipeline (`src/ffmpeg.ts`'s `run()`, `src/ingest.ts`, `src/moment-search.ts`, cooperative checks in `src/audio-analysis.ts`) giving real cancellation, an exactly-once retry guard (mirrors `web/lib/save-queue.ts`'s `beginPersisting` pattern), and conservative clean-up (terminal-only, removes only the job record, never the project directory); new cancel/retry/delete routes for both ingest and silences jobs plus a workspace-wide job list, all mutating routes behind `trustGuard` (#139). A new **Jobs** tab in the Config sidebar (`web/components/jobs-panel.tsx`) lists ingest jobs workspace-wide and the current project's silences jobs with per-status badges, live progress, and Cancel / Retry / Clean up (arm-then-confirm) / Open actions, 2s polling only while a job is running (#141). Take and cam ingest jobs are now genuinely cancellable too: `ffprobeJson`/`probe`/`probeAudio` accept an optional `signal`, `ingestTake`/`ingestCam` thread it through proxy/audio/transcribe phases, and `cancelIngestJob` no longer refuses composite-slug (take/cam) jobs (#142).
- **Deterministic media acceptance corpus + release gate** (PR #134, CRAFT-6186): `scripts/acceptance-corpus.ts` generates 5 reproducible ffmpeg-lavfi fixtures (SDR H.264 1080p, 4K HEVC 10-bit, VFR, portrait 9:16, long-sparse) into gitignored `fixtures/acceptance/`, with a manifest slot for an optional user-provisioned real-speech clip. `scripts/acceptance-gate.ts` runs real `ingest()` → a canonical dead-air cut → `exportCut()` → ffprobe structural verification (codec/container/pixfmt/dims/fps/duration/A+V/size) per fixture and emits a machine-readable report. A new CI `acceptance` job runs the gate offline behind `OPENKLIP_ACCEPTANCE=1`, reusing the CRAFT-6243 model-cache warm step. Perceptual SSIM/VMAF and a per-release thumbnail gallery are deferred (`docs/acceptance-corpus.md`).
- **Marketing site redesign + Fumadocs `/docs`** (PR #136): the `OPENKLIP_SITE=marketing` landing page was redesigned (nav, hero CTAs, supports strip, macOS-framed editor screenshot, theme toggle, Craftled footer credit); a Fumadocs `/docs` surface ships getting-started, file-model, CLI/MCP, and agent-workflow pages so the nav Docs link resolves to real content instead of a stub.
...
- **CI test suite no longer depends on live HuggingFace downloads** (PR #124, CRAFT-6243): new `src/model-env.mjs` pins the Transformers.js download cache to `OPENKLIP_MODEL_CACHE` and resolves models from a warmed cache as `localModelPath` when `TRANSFORMERS_OFFLINE`/`HF_HUB_OFFLINE=1`, retrying transient HF errors with backoff; `scripts/warm-models.mjs` warms the cache once, online. CI now caches and warms the model dir, then runs transcription/embedding steps fully offline, fixing repeat gateway-timeout failures on unrelated PRs.
- **Transactional, active-edit-safe force re-ingest** (PR #126, CRAFT-6181): force ingest now stages the replacement under a throwaway sibling slug, validates it, and swaps it into place under the same project lock `mutateProject` uses (`src/ingest-swap.ts`), restoring the original on any mid-swap failure instead of `rm -rf`-ing the live project directory first. Refuses to run while an agent task is active on the slug (checked before staging and again before the swap). Non-force ingest is unchanged.
- **Streamed asset uploads** (PR #127, CRAFT-6179): the asset upload route streams directly to disk (`writeUploadToFile`) instead of buffering the whole file with `file.arrayBuffer()`, closing an OOM path for large uploads; 413s on both the declared multipart size and the bytes actually written, with cleanup on every failure path.
- **Restored mobile Config access** (PR #128, CRAFT-6178): at a 390px viewport there was no reachable control for the Config view (its only trigger lived inside the left sidebar, itself unreachable on mobile). A new "Open config" mobile header button mirrors the existing Open Chat button; `test:integration` now runs all four browser tests (including the previously-unwired mobile test) in CI.
- **Stale GUI overlay saves can no longer overwrite CLI/MCP edits** (PR #129, CRAFT-6177): overlay add/update/remove now route through ID-scoped registry actions instead of submitting a whole-track replacement built from a client snapshot, so a concurrent CLI/MCP edit on the same track survives by construction; b-roll reorder (genuinely whole-track) keeps a CAS check (`expectedRevision`) that rejects a stale reorder before any write.
- **Recover cleanly from failed optimistic editor saves** (PR #130, CRAFT-6182): new `web/lib/save-queue.ts` state machine (queued → persisting → persisted/failed → retrying) with an exactly-once `beginPersisting` gate; a `SaveRecoveryBanner` shows a persistent error and dirty count with Retry and a confirm-gated Reload-from-disk, plus `beforeunload`/navigation guards while unsaved work remains.
- **Large CLI `--json` output no longer truncates when piped** (PR #131, CRAFT-6180): `tools --json`, `actions --json`, and `features --json` now route through a drain-aware `writeStdoutJson()` that awaits the `drain` event before returning, fixing silent truncation at Bun's 65,536-byte pipe buffer (the ~92KB tool manifest was hitting this).
- **Required golden-path browser test, edit to verified export** (PR #125, CRAFT-6184): `tests/golden-path-browser.test.ts` drives a real `next start` server through cut → server-action persistence → history → reload-survives-cut → export → structural ffprobe verification → a controlled 4xx failure, with page/console/network/hydration error capture. Wired into `test:integration` and the CI `integration` job.

### Documentation
- AGENTS.md documents the production launcher, loopback/local-trust follow-ons, durable job store, Job Center cancel/retry/clean-up surface, and the desktop shell stages inline. TODO.md Known Limitations and Completed checklist, README verified counts, CLAUDE.md release memory, and `docs/RELEASE-NOTES.md` are synced for this tag.
