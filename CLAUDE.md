...
- Code truth checked on 2026-07-06: `VERSION` and `package.json` are `0.41.1.2`; `bun test` runs 1904 tests (1901 pass, 3 skip without `OPENKLIP_INTEGRATION=1`); `src/agent-tools.ts` exposes 90 MCP tools; `openklip features --json` lists 51 capabilities from `src/features.ts`; `openklip actions --json` lists 44 registry actions.
...
- Published GitHub releases through `v0.41.1.1` (`gh release view v0.41.1.1`). `v0.41.1.2` is prepared as a draft in `docs/RELEASE-NOTES.md` from PR #80 and PR #81, but is not published until a tag/release is created.
