...
**Local-first.** Projects live under `projects/<slug>/` as plain files. Ingest transcribes with Transformers.js (Whisper). Export and proxies use bundled `ffmpeg-static` / `@ffprobe-installer/ffprobe`.
...
bun run build                          # required once before serve (production next start)
...
Six editor flows have optional headless Chrome tests that boot a real Next server against a fixture project: `tests/json-graphic-browser.test.ts`, `tests/transcript-diff-browser.test.ts`, `tests/mobile-overlays-browser.test.ts`, `tests/golden-path-browser.test.ts` (edit to verified export against a production `next start` server), `tests/save-recovery-browser.test.ts`, and `tests/job-center-browser.test.ts`. They skip in the default `bun test` run; `bun run test:integration` runs all six. To run one locally:
...
Durable, by-design constraints (not bugs):

- **Platform:** the full edit loop is **macOS-only**, and the desktop app is **Apple Silicon only** (no Intel/universal build yet). The CLI/MCP server can run elsewhere for read/query workflows.
- **Local-first, no telemetry:** everything is plain files on disk; nothing phones home.
- **GIF export** has no audio and uses a clamped width/fps/duration profile (a width override exists up to a 1920px ceiling; fps and duration are fixed).
- **Export "clipboard" destination** copies the exported file's path as text (browsers can't hold binary video), so no bytes are copied.
- **All export audio processing** (ducking, loudness, de-essing, seam crossfades) is export-only; preview audio is unprocessed.
- **Preview cut transitions** are a decorative WebGL sweep, not a pixel-accurate preview of the export-side ffmpeg transition; ffmpeg `xfade`/dip transitions are voice-only (fall back to a hard cut when overlays are present).
- **Rich/shader graphics** need WebGL2 (preview) and headless Chrome (export); there is no non-WebGL fallback renderer.
- **Whisper word timestamps** are ~100 ms loose; VAD snap tightens cuts to a 20 ms analysis-window resolution, not sample-accurate.
- **Multicam v1** ships ingest + follow/auto mixing + manual overrides; waveform auto-sync, shared-mic diarization, speaker-labeled captions, and NLE/FCPXML export are non-goals for v1.
- **Settings integrations** (ElevenLabs, Grok Voice, Reve) store and validate API keys only; nothing consumes them in the edit loop yet.

**Roadmap, tracked gaps, bugs, and feature requests** live in the **[OpenKlip Linear project](https://linear.app/craftled/project/openklip-687f57863d8c)** (team Craftled), the single source of truth for work items. The repo holds no TODO list.
