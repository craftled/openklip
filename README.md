...
- **Config shell + responsive panels**: right-side Config panel with a color temperature pad plus captions/timing controls; Settings → **Features** lists shipped capabilities (`web/lib/openklip-features.ts`); Chat and Config stay reachable below the desktop sidebar breakpoint via overlay buttons
