...
- **Action history**: append-only per-project log (`working/actions.jsonl`) records every user-facing mutation with actor, optional `authorId` / `model` / `agentSurface`, input/result summaries, timestamp, and revision before/after; History section with actor/action/task filters and per-entry **Show transcript diff** for transcript mutations; optional **Show edit attribution** in Settings → Appearance (default off) controls transcript hover and History author UI; `GET /api/projects/<slug>/history`; `openklip history` / MCP `history_list`
