...
**Current release:** v0.14.0.0 (2026-07-02 in `VERSION`, `package.json`, and `CHANGELOG.md`). Working local editor: cut → captions → cleanup review (filler/dead-air candidates) → b-roll → vignette → push-in zoom → titles → grade/LUT → rich graphics → json-render product announcement graphics → music placement → export audio quality (VAD snap, seam crossfades, ducking, loudness) → export; browser project creation (upload or drop a video onto the empty workspace); transcript phrase search with batch cuts and restore; real export settings (compression presets + output frame rate); append-only action history covering every user-facing mutation, with pre-mutation snapshots and task-level revert (`openklip revert`, MCP `revert`, GUI History panel); project brief with CLI/MCP/GUI parity; agent tasks with visible progress and cancel; `make-draft` one-prompt playbook; cinema player with live graphics/titles/captions overlays; resizable right chat sidebar with Claude MCP edits; right-side Config shell with color temperature controls; smaller-screen Chat and Config overlays; asset cards (`openklip analyze` + **Describe assets**); skills slash menu; MCP agent tools (68 tools); 38 registry actions; edit templates; native HTML/CSS graphics templates (pixel-faithful headless-Chrome export to ProRes 4444 alpha); default shadcn neutral theme with Base UI primitives; export dialog; macOS workspace folder picker; multi-agent filler cuts; sidebar asset bin with folder sync; written rationale notes on cuts/overlays; phrase-anchored cues that re-snap after a re-cut; multi-take assembly; 1117 tests; MIT.
...
- [x] **TDD test suite** (`bun test`: 1117 tests across actions, captions, EDL, exporter, project lock, chats, assets, workspace, agent-tools, templates, graphics, headless render, reanchor, multi-take assembly, product announcement, audio analysis, cleanup, cut snap, revert, and more)
