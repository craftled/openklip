...
**Current release:** v0.10.0.1 (2026-07-01); main has unreleased work since, including the done-for-you agent draft beta: a project brief (`brief.md`) agents read on every chat/edit request, an agent task model with live progress and a cancel button in the chat panel, and a `make-draft` playbook that turns one prompt into a full cut-captions-b-roll-music-export draft. Working local editor: cut → captions → cleanup review (filler/dead-air candidates) → b-roll → vignette → push-in zoom → titles → grade/LUT → rich graphics → json-render product announcement graphics → music placement → export audio quality (VAD snap, seam crossfades, ducking, loudness) → export; browser project creation (upload or drop a video onto the empty workspace); transcript phrase search with batch cuts and restore; real export settings (compression presets + output frame rate); append-only action history; project brief with CLI/MCP/GUI parity; agent tasks with visible progress and cancel; `make-draft` one-prompt playbook; cinema player with live graphics/titles/captions overlays; resizable right chat sidebar with Claude MCP edits; right-side Config shell with color temperature controls; smaller-screen Chat and Config overlays; asset cards (`openklip analyze` + **Describe assets**); skills slash menu; MCP agent tools (67 tools); 38 registry actions; edit templates; native HTML/CSS graphics templates (pixel-faithful headless-Chrome export to ProRes 4444 alpha); default shadcn neutral theme with Base UI primitives; export dialog; macOS workspace folder picker; multi-agent filler cuts; sidebar asset bin with folder sync; written rationale notes on cuts/overlays; phrase-anchored cues that re-snap after a re-cut; multi-take assembly; 1014 tests; MIT.
...
- Dead-air removal has no GUI undo affordance yet: `dead-air-rm` is CLI/MCP only (the Cleanup panel only adds spans, never removes a registered one).
...
- Only the `make-draft` playbook is written; `make-short` and `revise-draft` (Milestone 2.2) do not exist yet.
...
- Action history logs registry and GUI mutations, plus MCP `brief_set` as a best-effort `brief-set` entry with no EDL revision bump. Non-registry CLI paths (asset registration, template set, assembly, `openklip brief --set`) and GUI brief saves do not log yet. History has no filters and no undo. Multi-take `assemble` also rewrites `project.json` outside `mutateProject`, resetting the revision counter without a history entry.
