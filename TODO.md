...
- [x] **Scene crop mode + revise-draft convert-to-short** (v0.22.0.0): `cropMode: manual|scene` on `project.export` with `src/auto-crop.ts`; GUI Manual/Scene toggle; `revise-draft` section 3b for Shorts/Reels/TikTok without reverting. Verification 2026-07-03: `tests/auto-crop.test.ts`, `tests/templates.test.ts`, 1303 tests green.
...
- [x] Scene crop mode (`cropMode: scene`): when `openklip analyze` has written a `sceneLog`, export-set in scene mode derives crop focus from speaker spans (center-weighted today; per-segment focus is a future upgrade).
- [ ] Face/saliency auto-crop beyond scene-log center weighting.
