...
- Agents have no MCP or CLI tool to query action history or past task ids; only the GUI History panel (`GET /api/projects/<slug>/history`) and `openklip revert` itself read `working/actions.jsonl`. The `revise-draft` playbook works from its own session context and can only reuse a task id it saw earlier in the same conversation.
