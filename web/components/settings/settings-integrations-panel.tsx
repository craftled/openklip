...
function ReveIntegrationRow({
  ...
  const onSave = async () => {
    ...
    setIsSaving(true);
    try {
      onStatusChange(await saveReveApiKey(trimmedApiKey));
      setApiKey("");
    } catch (e) {
      setError((e as Error).message);
    } finally {
  ...
  const onClear = async () => {
    ...
    setIsSaving(true);
    try {
      onStatusChange(await clearReveApiKey());
      setApiKey("");
    } catch (e) {
      setError((e as Error).message);
    } finally {
  ...
  const onTest = async () => {
    ...
    setIsTesting(true);
    try {
      setTestStatus(await testReveApiKey(trimmedApiKey || undefined));
    } catch (e) {
      setError((e as Error).message);
    } finally {
      setIsTesting(false);
  ...
  return (
    <SettingsRow
      description="Paste a Reve API key for image generation."
      title={
        <span className="flex items-center gap-2">
          <span>Reve</span>
          <Badge
